...
train_data = np.array(train_data).reshape(int(len(wave_data) * (1 - test_valid_ratio)), 16, 20, 1)
valid_data = np.array(valid_data).reshape(int(len(wave_data) * test_valid_ratio * (1 - test_ratio)), 16, 20, 1)
test_data = np.array(test_data).reshape(int(len(wave_data) * test_valid_ratio * test_ratio), 16, 20, 1)
...
model = Sequential()
model.add(Conv2D(num_filters, kernel_size_1, padding='valid', activation='relu'))
model.add(Conv2D(num_filters, kernel_size_2, padding='valid', activation='relu'))
model.add(Conv2D(num_filters, kernel_size_3, padding='valid', activation='relu'))
